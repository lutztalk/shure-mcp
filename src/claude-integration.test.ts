...
test("Claude integration JSON examples are parseable and point at shure-mcp", () => {
  ...
  assert.equal(desktopConfig.mcpServers.shure.command, "node");
  assert.match(desktopConfig.mcpServers.shure.args[0], /dist\/index\.js$/);
  assert.equal(codeConfig.mcpServers.shure.command, "node");
